...
if(isset($_POST['uid']))
{
  $queryy=mysqli_query($con,"select * from bitparticle where id=".$_POST['uid']."");
...
}else if(isset($_POST['id']))
{
 ...
 if(isset($_FILES['themeimg']))
 {
   ...
   $img_path=getcwd()."/Images/bitparticle/themes/".$name2;
   $img_url="http://$_SERVER[HTTP_HOST]"."/lyrically/Images/bitparticle/themes/".$name;

   move_uploaded_file($_FILES['themeimg']['tmp_name'],$img_path);

   mysqli_query($con,"update bitparticle set theme_path='".$img_path."',theme_url='".$img_url."' where id=".$_POST['id']."");
 }
 ...
 if(isset($_FILES['thumbimg']))
 {
   ...
   $thumb_path=getcwd()."/Images/bitparticle/thumb/".$name;
   $thumb_url="http://$_SERVER[HTTP_HOST]"."/lyrically/Images/bitparticle/thumb/".$name;

   move_uploaded_file($_FILES['thumbimg']['tmp_name'],$thumb_path);

   mysqli_query($con,"update bitparticle set thumb_path='".$thumb_path."',thumb_url='".$thumb_url."' where id=".$_POST['id']."");
 }
 ...
 if(isset($_FILES['particle']))
 {
   ...
   $file_path=getcwd()."/Images/bitparticle/files/".$namee;
   $file_url="http://$_SERVER[HTTP_HOST]"."/lyrically/Images/bitparticle/files/".$name;

   move_uploaded_file($_FILES['particle']['tmp_name'],$file_path);

   mysqli_query($con,"update bitparticle set particle_path='".$file_path."',particle_url='".$file_url."',prefab_name='".$filename."' where id=".$_POST['id']."");
 }

 /*if(isset($_POST['category_id']))
 {
   mysqli_query($con,"update bitparticle set category_id=".$_POST['category_id']." where id=".$_POST['id']."");
 }*/

 if(isset($_POST['theme_name']))
 {
   mysqli_query($con,"update bitparticle set theme_name='".$_POST['theme_name']."' where id=".$_POST['id']."");
 }

 if(isset($_POST['prefab_name']))
 {
   mysqli_query($con,"update bitparticle set prefab_name='".$_POST['prefab_name']."' where id=".$_POST['id']."");
 }
...
}else if(isset($_POST['theme_name']))
{
  ...
  $file_path=getcwd()."/Images/bitparticle/files/".$namee;
  $file_url="http://$_SERVER[HTTP_HOST]"."/lyrically/Images/bitparticle/files/".$namee;
  
  $img_path=getcwd()."/Images/bitparticle/themes/".$name2;
  $img_url="http://$_SERVER[HTTP_HOST]"."/lyrically/Images/bitparticle/themes/".$name2;
  ...
  move_uploaded_file($_FILES['themeimg']['tmp_name'],$img_path);
   ...
   $queryy=mysqli_query($con,"select * from bitparticle order by position desc limit 1");
  ...
  mysqli_query($con,"insert into bitparticle values(0,".$category_id.",'".$theme_name."','".$filename."','".$img_path."','".$img_url."','".$thumb_path."','".$thumb_url."','".$file_path."','".$file_url."',".$pos.")");
...
<body class="fixed-nav sticky-footer bg-dark" id="page-top" onload="get_custom_data('save_bitparticle','show_bitparticle',0,'',0,0);">
  <div class="container-fluid" >
    <div class="card card-register mx-auto mt-5">
      ...
      <div class="card-body">
       ...
       <form name="myform" method="POST" enctype="multipart/form-data" action="save_bit_particle.php">
