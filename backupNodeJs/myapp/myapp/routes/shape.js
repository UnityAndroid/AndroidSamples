...
router.get('/', tokenn.accessToken,async function (req, res) {
    ...
    var qry = "SELECT * FROM `tbl_shape` order by shape_id desc limit ?,?";
...
router.post('/saveorupdate', tokenn.accessToken, function (req, res) {
    var name = req.body.shape;
    if (req.body.shape_id) {
        var id = req.body.shape_id;
        var qry = "UPDATE tbl_shape SET shape = ? WHERE shape_id = ?";
        var finalqry = mysql.format(qry, [name, id]);
    } else {
        var qry = "INSERT INTO tbl_shape(shape) VALUES(?)";
...
router.post('/delete', tokenn.accessToken, function (req, res) {
    var id = req.body.shape_id;
    var qry = " DELETE FROM tbl_shape WHERE shape_id=?";
