...
router.get('/', tokenn.accessToken,async function (req, res) {
    ...
    var qry = "SELECT * FROM `tbl_purity`";
    ...
    var qry = "SELECT * FROM `tbl_purity` order by purity_id desc limit ?,?";
...
router.post('/saveorupdate', tokenn.accessToken, function (req, res) {
    var name = req.body.purity;
    if (req.body.purity_id) {
        var id = req.body.purity_id;
        var qry = "UPDATE tbl_purity SET purity = ? WHERE purity_id = ?";
        var finalqry = mysql.format(qry, [name, id]);
    } else {
        var qry = "INSERT INTO tbl_purity(purity) VALUES(?)";
...
router.post('/delete', tokenn.accessToken, function (req, res) {
    var id = req.body.purity_id;
    var qry = " DELETE FROM tbl_purity WHERE purity_id=?";
