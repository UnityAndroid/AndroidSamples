...
router.get('/', tokenn.accessToken, async function (req, res) {
    ...
    var qry = "SELECT * FROM `tbl_brand` order by brand_id desc limit ?,?";
...
router.post('/saveorupdate', tokenn.accessToken, function (req, res) {
    ...
    if (req.body.brand_id) {
        ...
        var qry = "UPDATE tbl_brand SET brand = ? WHERE brand_id = ?";
        var finalqry = mysql.format(qry, [name, id]);
    } else {
        var qry = "INSERT INTO tbl_brand(brand) VALUES(?)";
...
router.post('/delete', tokenn.accessToken, function (req, res) {
    var id = req.body.brand_id;
    var qry = " DELETE FROM tbl_brand WHERE brand_id=?";
