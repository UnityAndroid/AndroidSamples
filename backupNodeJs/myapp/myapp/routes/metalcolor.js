...
router.get('/', tokenn.accessToken,async function (req, res) {
    ...
    var qry = "SELECT * FROM `tbl_metal_color`";
    ...
    var qry = "SELECT * FROM `tbl_metal_color` order by metal_color_id desc limit ?,?";
...
router.post('/saveorupdate', tokenn.accessToken, function (req, res) {
    var name = req.body.metal_color_name;
    if (req.body.metal_color_id) {
        var id = req.body.metal_color_id;
        var qry = "UPDATE tbl_metal_color SET metal_color_name = ? WHERE metal_color_id = ?";
        var finalqry = mysql.format(qry, [name, id]);
    } else {
        var qry = "INSERT INTO tbl_metal_color(metal_color_name) VALUES(?)";
...
router.post('/delete', tokenn.accessToken, function (req, res) {
    var id = req.body.metal_color_id;
    var qry = " DELETE FROM tbl_metal_color WHERE metal_color_id=?";
