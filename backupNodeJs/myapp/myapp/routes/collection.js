...
router.get('/', tokenn.accessToken,async function (req, res) {
    ...
    var qry = "SELECT * FROM `tbl_collection`";
    ...
    var qry = "SELECT * FROM `tbl_collection` order by collection_id desc limit ?,?";
...
router.post('/saveorupdate', tokenn.accessToken, function (req, res) {
    ...
    if (req.body.collection_id) {
        ...
        var qry = "UPDATE tbl_collection SET collection = ? WHERE collection_id = ?";
        var finalqry = mysql.format(qry, [name, id]);
    } else {
        var qry = "INSERT INTO tbl_collection(collection) VALUES(?)";
...
router.post('/delete', tokenn.accessToken, function (req, res) {
    var id = req.body.collection_id;
    var qry = " DELETE FROM tbl_collection WHERE collection_id=?";
