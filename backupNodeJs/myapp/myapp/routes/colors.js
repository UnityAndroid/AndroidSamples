...
router.get('/', tokenn.accessToken,async function (req, res) {
    ...
    var qry = "SELECT * FROM `tbl_color` order by color_id desc limit ?,?";
...
router.post('/saveorupdate', tokenn.accessToken, function (req, res) {
    var name = req.body.color_name;
    if (req.body.color_id) {
        var id = req.body.color_id;
        var qry = "UPDATE tbl_color SET color_name = ? WHERE color_id = ?";
        var finalqry = mysql.format(qry, [name, id]);
    } else {
        var qry = "INSERT INTO tbl_color(color_name) VALUES(?)";
...
router.post('/delete', tokenn.accessToken, function (req, res) {
    var id = req.body.color_id;
    var qry = " DELETE FROM tbl_color WHERE color_id=?";
